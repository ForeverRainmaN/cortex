...
class HandleCommandSpec extends AsyncFlatSpec, AsyncIOSpec, Matchers:
  ...
  it should "handle enqueue command" in:
    for
      ...
      _      <- handleCommand(store)(contentId, Command.Enqueue(contentId, Book))
      events <- store.loadById(contentId)
    yield events shouldBe List(ContentQueued(contentId, Book))
  ...
  it should "handle start command" in:
    for
      ...
      events <- store.loadById(contentId)
    yield events shouldBe List(ContentQueued(contentId, Book), ContentStarted(contentId))

  it should "return Right for successful command" in:
    for
      store  <- InMemoryEventStore.make[IO]
      result <- handleCommand(store)(contentId, Command.Enqueue(contentId, Book))
  ...
  it should "not append event and return Left when command is invalid" in:
    ...
    yield
      ...
      events shouldBe List(ContentQueued(contentId, Book), ContentStarted(contentId))
  ...
  it should "return Left(AlreadyExists) when enqueueing existing content" in:
    ...
    yield
      ...
      events shouldBe List(ContentQueued(contentId, Book))
